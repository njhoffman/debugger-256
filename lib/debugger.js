...
const debug = (level, subsystem, ...messages) => {
  ...
  _.each(_.keys(beautifiers), b => {
    _.each(messages, msg => {
      if (_.keys(msg).indexOf(b) !== -1) {
        ...
        prettyLine[0] += _.isArray(pretty) ? pretty[0] : pretty;
        pretty[1] && _.merge(prettyLine[1], pretty[1]);
      }
    });
  });
  ...
  // remove empty objects or objects tagged with log level higher (more verbose) than current log level
  messages.forEach(msg => msg._log_level && console.log(msg._log_level + " " + confLevel));
  _.remove(messages, msg => _.isEmpty(msg) || msg._log_level > confLevel);
