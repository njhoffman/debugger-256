...
const readConfFile = (loc) => {
  ...
  if (_.has(confFile, `subsystems._env.${process.env.NODE_ENV}`)) {
    ...
    confFile = _.assign(confFile, envConf);
  }
  if (_.has(confFile, 'settings')) {
    _.merge(options, confFile.settings);
  }
  return confFile;
};
...
const loadConfFile = () => {
  ...
  while (i > 0) {
    ...
    if (checkConfLocation(file1)) {
      conf = readConfFile(file1);
      return conf;
    } else if (checkConfLocation(file2)) {
      conf = readConfFile(file2);
      return conf;
    }
    i--;
  }
