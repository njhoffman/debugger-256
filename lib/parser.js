...
const parseMessage = (messages, subsystem, render) => {
  const ssLength = getSubsystems().length > 0
    ? maxBy(getSubsystems(), (ss) => ss.length).length + 1
    : subsystem.length;
  let out = '';
    ...
    if (isObject(message)) {
      ...
      if (/%.*%/.test(message) && nextMsg && nextMsg['color']) {
        ...
        message.match(/(%.*?%)/g).forEach(colorMsg => {
          difference(colorTags, parsedTags).some(colorTag => {
            let renderObj = {};
            if (!isUndefined(getOptions().customColors[colorTag])) {
        ...
        difference(parsedTags, colorTags).forEach(diffTag => {
