...
module.exports = (settings) => {
  const response =  (response, render) => {
    // express response object
    const { code, time, url, contentLength } = response;
    let pretty = '';
  ...
  const webpackAsset = (asset, render) => {
    const sizeColor = asset.size > 1000000
      ? 'orange' : asset.overLimit
        ? 'yellow' : 'white';
  ...
  const webpackBuildSummary = (webpackInfo, render) => { };
